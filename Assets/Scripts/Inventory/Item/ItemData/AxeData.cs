...
public class AxeData : ToolData
{
    ...
    public override void UseItem(Transform user){
        PlayerStateManager player = user.GetComponentInParent<PlayerStateManager>();
        if (Physics.Raycast(user.position, user.TransformDirection(Vector3.forward), out _hit, 7.5f, LayerMask.GetMask("Interactable"))){      
            _hit.collider.transform.gameObject.GetComponentInParent<IChopable>().TakeAxeDamage(ToolDamage * player.BenchPress, _hit);
        } else{
            throw new ItemNotUsedException();
        }
    }
